...
cy.get('input[name="rooms.0.childCount"]')
  ...
  // .should('have.value', '1');
    ...
    cy.contains('button[role="combobox"]', 'expedia')
      ...
      cy.wait(2000);
      cy.get('input[name="first_name"]').type('dhammi');
      cy.get('input[name="last_name"]').type('Doe');
      cy.get('input[name="email"]').type('dhammi@example.com');
      cy.get('input[name="mobile"]').type('3434566885');
    ...
    //cy.contains('Save Booking').should('not.be.disabled').click();
